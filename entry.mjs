						...
						if ('}' !== E)
							if (':' !== E)
								if ('(' !== E) t.push({ type: 'CHAR', index: F, value: u[F++] })
									...
									for (; s < u.length; ) {
										if (!A(u[s], !1)) {
											n("Invalid character '" + u[s] + "' at " + s + '.'), (a = !0)
											break
										}
										if ('\\' !== u[s]) {
											if (')' === u[s]) {
								...
								for (var B = '', o = F + 1; o < u.length; ) {
									var h = u.substr(o, 1)
									if (!((o === F + 1 && e.test(h)) || (o !== F + 1 && C.test(h)))) break
									B += u[o++]
								}
								if (!B) {
	...
	for (
		var t = E(u),
			...
			h = function (u) {
				...
				var F = t[i]
				throw new TypeError('Unexpected ' + F.type + ' at ' + F.index + ', expected ' + u)
			},
			p = function () {
		...
		if ((l || m || !B('ASTERISK') || (m = '.*'), l || m)) {
			var d = f || ''
			;-1 === e.indexOf(d) && ((s += d), (d = '')), s && (A.push(c(s)), (s = ''))
			var g = l || n++
			if (a.has(g)) throw new TypeError("Duplicate name '" + g + "'.")
			a.add(g), A.push({ name: g, prefix: c(d), suffix: '', pattern: m || C, modifier: o() || '' })
		} else {
...
function a(u, D, t) {
	return u instanceof RegExp
		? (function (u, D) {
				if (!D) return u
				for (var t = /\((?:\?<(.*?)>)?(?!\?)/g, F = 0, e = t.exec(u.source); e; )
					D.push({ name: e[1] || F++, prefix: '', suffix: '', modifier: '', pattern: '' }),
						(e = t.exec(u.source))
				return u
		  })(u, D)
		: Array.isArray(u)
...
function c(u) {
	return !(!u || u.length < 2 || ('[' !== u[0] && (('\\' !== u[0] && '{' !== u[0]) || '[' !== u[1])))
}
var f,
...
function v(u) {
	if ('' === u) return u
	if (/[\t\n\r #%/:<>?@[\]^\\|]/g.test(u)) throw new TypeError("Invalid hostname '" + u + "'")
	var D = new URL('https://example.com')
	return (D.hostname = u), D.hostname
}
function y(u) {
	if ('' === u) return u
	if (/[^0-9a-fA-F[\]:]/g.test(u)) throw new TypeError("Invalid IPv6 hostname '" + u + "'")
		...
		return (
						...
						if (this.state === f.INIT) {
							this.rewind(),
								this.isHashPrefix()
									...
									: this.isSearchPrefix()
									? (this.changeState(f.SEARCH, 1), (this.internalResult.hash = ''))
						...
						switch (this.state) {
							...
							case f.AUTHORITY:
								this.isIdentityTerminator()
									...
									: (this.isPathnameStart() || this.isSearchPrefix() || this.isHashPrefix()) &&
									  this.rewindAndSetState(f.HOSTNAME)
								break
							case f.USERNAME:
			...
			(t.safeToken = function (u) {
				return (
					...
					u < this.tokenList.length ? this.tokenList[u] : this.tokenList[this.tokenList.length - 1]
				)
			}),
			(t.isNonSpecialPatternChar = function (u, D) {
				var t = this.safeToken(u)
				return t.value === D && ('CHAR' === t.type || 'ESCAPED_CHAR' === t.type || 'INVALID_CHAR' === t.type)
			}),
			...
			(t.isSearchPrefix = function () {
				...
				return 'NAME' !== u.type && 'PATTERN' !== u.type && 'CLOSE' !== u.type && 'ASTERISK' !== u.type
			}),
...
function O(u, D) {
	if ('string' != typeof u) throw new TypeError("parameter 1 is not of type 'string'.")
	var t = new URL(u, D)
	return {
...
function H(u, D, t) {
	...
	if ('string' == typeof D.baseURL)
		try {
			;(F = new URL(D.baseURL)),
				(u.protocol = F.protocol ? F.protocol.substring(0, F.protocol.length - 1) : ''),
	...
	if (
		('string' == typeof D.protocol &&
			(u.protocol = (function (u, D) {
				var t
				return (u = (t = u).endsWith(':') ? t.substr(0, t.length - ':'.length) : t), D || '' === u ? u : g(u)
			})(D.protocol, t)),
		...
		u.pathname = (function (u, D, t) {
			...
			var F = '/' == u[0]
			return (
				(u = new URL(F ? u : '/-' + u, 'https://example.com').pathname), F || (u = u.substring(2, u.length)), u
			)
	...
	return (
		'string' == typeof D.search &&
			(u.search = (function (u, D) {
				...
				var t = new URL('https://example.com')
				return (t.search = u), t.search ? t.search.substring(1, t.search.length) : ''
			})(D.search, t)),
		...
		if ('string' != typeof A)
			if ('' !== A.pattern) {
				var r = 'number' != typeof A.name,
					...
					s = '' !== A.suffix || ('' !== A.prefix && (1 !== A.prefix.length || !i.includes(A.prefix)))
				s ||
					!r ||
					A.pattern !== t ||
					'' !== A.modifier ||
					...
					n.prefix ||
					n.suffix ||
					(s = 'string' == typeof n ? F.test(n.length > 0 ? n[0] : '') : 'number' == typeof n.name),
					!s &&
						'' === A.prefix &&
						E &&
						'string' == typeof E &&
					...
					r && (e += ':' + A.name),
					'.*' === A.pattern
						? (e += r || (E && 'string' != typeof E && !E.modifier && !s && '' === A.prefix) ? '(.*)' : '*')
						: A.pattern === t
						? r || (e += '(' + t + ')')
						: (e += '(' + A.pattern + ')'),
					A.pattern === t && r && '' !== A.suffix && F.test(A.suffix[0]) && (e += '\\'),
					(e += N(A.suffix)),
					s && (e += '}'),
					(e += A.modifier)
			} else {
...
var U = (function () {
		...
		try {
			if ('string' == typeof u) {
				...
				if ((t.parse(), (u = t.result), D)) {
					if ('string' != typeof D) throw new TypeError("'baseURL' parameter is not of type 'string'.")
					u.baseURL = D
			...
			;(this.pattern = H(
				...
				d(this.pattern.protocol) === this.pattern.port && (this.pattern.port = '')
			for (var C, A = F(I); !(C = A()).done; )
				if ((e = C.value) in this.pattern) {
					...
					try {
						var i = n(r, E)
						;(this.regexp[e] = s(i, this.keys[e], E)), (this.component_pattern[e] = L(i, E))
					} catch (u) {
						throw new TypeError('invalid ' + e + " pattern '" + this.pattern[e] + "'.")
					}
				}
		} catch (u) {
	...
	return (
		(t.test = function (u, D) {
			...
			if ('string' != typeof u && D) throw new TypeError("parameter 1 is not of type 'string'.")
		...
		(t.exec = function (u, D) {
			...
			if (void 0 !== u) {
				...
				for (e in ((C.inputs = D ? [u, D] : [u]), this.pattern)) {
					...
					if (!A) return null
					for (var E, n = {}, r = F(this.keys[e].entries()); !(E = r()).done; ) {
						var i = E.value,
							s = i[1]
						;('string' != typeof s.name && 'number' != typeof s.name) || (n[s.name] = A[i[0] + 1] || '')
					}
...
const initCustomElementRegistry = (target, exclude) => {
	...
	const customElements = target.customElements || (target.customElements = new CustomElementRegistry())
...
class Document extends Node {
	createElement(name) {
		...
		const TypeOfHTMLElement =
			...
			(customElementInternals && customElementInternals.constructorByName.get(name)) ||
			HTMLUnknownElement
...
const getImageType = (type) =>
	type === 'image/avif' || type === 'image/jpeg' || type === 'image/png' || type === 'image/webp' ? type : 'image/png'
...
function responseIterator(response) {
	...
	if (isBuffer(body)) body = Readable.from(body)
...
const pageMap = new Map([
	...
	['src/pages/bookmark.astro', _page2],
	['src/pages/rss.xml.js', _page3],
	['src/pages/secreto.astro', _page4],
...
const _manifest = Object.assign(
	deserializeManifest({
		...
		routes: [
			...
			{
				file: '',
				links: ['_astro/_...page_.352d9575.css'],
				scripts: [
					{ type: 'external', value: '_astro/page.1504cc3a.js' },
					{
						stage: 'head-inline',
			...
			},
			{
				...
				routeData: {
					...
					type: 'page',
					pattern: '^\\/bookmark\\/?$',
					segments: [[{ content: 'bookmark', dynamic: false, spread: false }]],
					params: [],
					component: 'src/pages/bookmark.astro',
					pathname: '/bookmark',
					prerender: false,
					_meta: { trailingSlash: 'ignore' },
				},
			...
			},
			{
				...
				routeData: {
					...
					type: 'page',
					pattern: '^\\/secreto\\/?$',
					segments: [[{ content: 'secreto', dynamic: false, spread: false }]],
					params: [],
					component: 'src/pages/secreto.astro',
					pathname: '/secreto',
					prerender: false,
					_meta: { trailingSlash: 'ignore' },
				},
			...
			},
			{
				...
				routeData: {
					...
					segments: [
						...
						[{ content: 'category', dynamic: true, spread: false }],
						[{ content: '...page', dynamic: true, spread: true }],
					],
					params: ['...categories', 'category', '...page'],
					component: 'src/pages/[...categories]/[category]/[...page].astro',
					prerender: false,
					_meta: { trailingSlash: 'ignore' },
				},
			},
			{
				...
				routeData: {
					...
					pattern: '^(?:\\/(.*?))?\\/([^/]+?)\\/?$',
					segments: [
						[{ content: '...blog', dynamic: true, spread: true }],
		...
		entryModules: {
			...
			'D:/CRACK/DEV/basement/public/krido.jpg': 'chunks/krido.5035a6b2.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/arabella.jpg': 'chunks/arabella.f9d48940.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/banyak-tahu.jpg': 'chunks/banyak-tahu.197368c5.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/editor.jpg': 'chunks/editor.a011f04f.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/emptyness.jpg': 'chunks/emptyness.1bfb253a.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/fud.jpg': 'chunks/fud.ab378ef4.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/konfigurasi-vscode/hasil.png': 'chunks/hasil.58cfa885.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/konfigurasi-vscode/vscode-default.png':
				'chunks/vscode-default.2ed24e37.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/konsumen-pintar.jpg': 'chunks/konsumen-pintar.f8b67d2d.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/orm.png': 'chunks/orm.4df207b3.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/que-sera.jpg': 'chunks/que-sera.1f6d83ad.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/see-you.jpg': 'chunks/see-you.7fa9f4ce.mjs',
			'D:/CRACK/DEV/basement/src/assets/images/stabilitas.jpg': 'chunks/stabilitas.4cfd178a.mjs',
			'~/components/widgets/Bookmark.svelte': '_astro/Bookmark.c758d8b9.js',
			'@astrojs/svelte/client.js': '_astro/client.c4e17359.js',
			'astro:scripts/page.js': '_astro/page.1504cc3a.js',
			'~/components/widgets/Secreto.svelte': '_astro/Secreto.d924f721.js',
			'astro:scripts/before-hydration.js': '',
		},
		assets: [
			'/_astro/krido.3734aec9.jpg',
			'/_astro/gradient.c6552caf.jpg',
			'/_astro/inter-cyrillic-variable-wghtOnly-normal.262a1054.woff2',
			'/_astro/inter-vietnamese-variable-wghtOnly-normal.ac4e131c.woff2',
			'/_astro/inter-cyrillic-ext-variable-wghtOnly-normal.848492d3.woff2',
			'/_astro/inter-greek-variable-wghtOnly-normal.89b4a3fe.woff2',
			'/_astro/inter-latin-variable-wghtOnly-normal.450f3ba4.woff2',
			'/_astro/inter-latin-ext-variable-wghtOnly-normal.45606f83.woff2',
			'/_astro/inter-greek-ext-variable-wghtOnly-normal.fe977ddb.woff2',
			'/_astro/hasil.57abd1f6.png',
			'/_astro/arabella.1804b29a.jpg',
			'/_astro/vscode-default.5f75c929.png',
			'/_astro/orm.87c3b74c.png',
			'/_astro/que-sera.daef5415.jpg',
			'/_astro/emptyness.742f02c9.jpg',
			'/_astro/banyak-tahu.afd33e6a.jpg',
			'/_astro/editor.1b9a380f.jpg',
			'/_astro/fud.beecc239.jpg',
			'/_astro/see-you.11fe0ec5.jpg',
			'/_astro/stabilitas.f55cda79.jpg',
			'/_astro/konsumen-pintar.6ec27bb3.jpg',
			'/_astro/_...page_.352d9575.css',
			'/favicon.ico',
			'/favicon.svg',
			'/gradient.jpg',
			'/krido.jpg',
			'/loading.svg',
			'/robot.txt',
			'/_astro/Bookmark.c758d8b9.js',
			'/_astro/client.c4e17359.js',
			'/_astro/page.1504cc3a.js',
			'/_astro/Secreto.d924f721.js',
			'/_astro/supabase.3a92e09f.js',
			'/fonts/Basement/BasementGrotesque-Black.woff2',
			'/fonts/Basement/BasementGrotesque-Display.woff2',
			'/fonts/Grotesk/SpaceGrotesk-Bold.woff2',
			'/fonts/Grotesk/SpaceGrotesk-Light.woff2',
			'/fonts/Grotesk/SpaceGrotesk-Medium.woff2',
			'/fonts/Grotesk/SpaceGrotesk-Regular.woff2',
			'/fonts/Grotesk/SpaceGrotesk-SemiBold.woff2',
			'/fonts/Inter/Inter-Regular.woff2',
			'/fonts/Neue/Neue-Montreal-Light.woff2',
			'/fonts/Neue/Neue-Montreal-Medium.woff2',
			'/fonts/Neue/Neue-Montreal-Regular-400.woff2',
			'/fonts/Space/SpaceMono-Regular.woff2',
			'/_astro/page.1504cc3a.js',
...
const _args = {
	mode: 'standalone',
	client: 'file:///D:/CRACK/DEV/basement/dist/client/',
	server: 'file:///D:/CRACK/DEV/basement/server',
	host: false,
	port: 3000,
}
